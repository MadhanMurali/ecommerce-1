...
const BaseUrl = 'https://www.healthcarecrm.vimkes.com/';
...
const stringToDateTime = (string_date) => {
  ...
  if (bits[2] === "0999" || bits[2] === "8999" || bits[2] === "1000" || bits[2] === "9000") {
    mydate = '';
  }
...
const stringToDateTime1 = (string_date) => {
  ...
  if (bits[0] === "0999" || bits[0] === "8999" || bits[0] === "1000" || bits[0] === "9000") {
    mydate = '';
  }
  if ((mydate.toString()) === "Invalid Date") {
...
const stringToDate = (string_date) => {
  ...
  try {
    ...
    if (parts[2] === "0999" || parts[2] === "8999" || parts[2] === "1000" || parts[2] === "9000") {
      mydate = '';
    }
    if ((mydate.toString()) === "Invalid Date") {
      return '';
...
const plusActionCart = (d) => {
  ...
  if (arr.length === 0) {
    ...
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].item_id === d.id) {
        arr[i].item_id = d.id;
        arr[i].quantity = (d.quantity * 1) + 1;
      } else {
        const newItem = ()=> {
          return {
            item_id: d.id,
...
const numberFormat = (num) => {
  ...
  if (num == null || num == "null") {
    ...
    if (isNaN(num)) {
      ...
      var str = res.split(".");
      res = str[0].replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") + (str[1] ? ("." + str[1]) : "");
      return '\u20B9 ' + res
    }
